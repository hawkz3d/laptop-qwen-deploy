# -*- coding: utf-8 -*-
"""b8600 + -ncmoe N：清日志启动，等加载完成，测显存 + 稳态速度"""
...
try:
    ...
    while time.time() - t0 < 420:
        ...
        if 'model loaded' in log or 'CUDA error' in log or 'error:' in log:
            break
        time.sleep(8)
    print('--- loaded in', round(time.time()-t0, 0), 's ---')
    for line in log.splitlines():
        if any(k in line.lower() for k in ['buffer size', 'offloaded', 'cuda error', 'model loaded', 'listening', 'kv buffer', 'recurrent']):
            print(line)
finally:
    ssh.close()
