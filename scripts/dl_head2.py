# -*- coding: utf-8 -*-
"""Range 下载 satgeze 1M Q4_K_M 前 16MB，检查 KV keys + tensor 名（fused/chat_template）"""
import sys, struct, io
sys.stdout.reconfigure(encoding='utf-8', errors='replace')
import paramiko

URL = 'https://hf-mirror.com/satgeze/Ornith-1.0-35B-1M-GGUF/resolve/main/ornith-1.0-35b-1M-MTP-Q4_K_M.gguf'
REMOTE_HEAD = 'D:/models/sat_head.gguf'
...
try:
    ...
    print(f'tensor gate_exps={len(gate)} up_exps={len(up)} fused={len(fused_t)}')
    for n in names[:50]:
        if any(k in n for k in ('ffn_gate', 'ffn_up', 'fused')):
            print('  ', n)
finally:
    ssh.close()
