# -*- coding: utf-8 -*-
"""前台测试 b8600 -c 196608 -fit off，看 n_ctx 是否生效 + 是否崩溃"""
import sys, time
...
try:
    ...
    while time.time() - t0 < 115:
        if stdout.channel.recv_ready():
            ...
            time.sleep(0.2)
        if b'model loaded' in buf or b'CUDA error' in buf or b'error' in buf.lower():
            time.sleep(2)
    ...
    for line in text.splitlines():
        if any(k in line.lower() for k in ['n_ctx', 'kv buffer', 'fit', 'cuda error', 'error', 'model loaded', 'listening', 'ctx_size', 'memory']):
            print(line)
    ssh.exec_command('powershell -NoProfile -Command "Get-Process llama-server -ErrorAction SilentlyContinue | Stop-Process -Force"', timeout=20)[1].read()
finally:
    ssh.close()
