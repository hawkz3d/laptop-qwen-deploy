# -*- coding: utf-8 -*-
"""Range 下载 Qwen3.6 APEX Quality 头，检查架构/fused/chat_template"""
import sys, struct, io
sys.stdout.reconfigure(encoding='utf-8', errors='replace')
import paramiko

URL = 'https://hf-mirror.com/mudler/Qwen3.6-35B-A3B-APEX-GGUF/resolve/main/Qwen3.6-35B-A3B-APEX-Quality.gguf'
REMOTE_HEAD = 'D:/models/qwen36_head.gguf'
...
try:
    ...
    for i in range(kv_count):
        ...
        read_value(b, t)
        keys.append(key)
    arch = [k for k in keys if 'architecture' in k]
    print('arch:', arch)
    print('HAS chat_template:', any('chat_template' in k for k in keys))
    print('HAS compat/fused key:', any('compat' in k.lower() or 'fused' in k.lower() or 'fuse' in k.lower() for k in keys))
    for k in keys:
        if 'compat' in k.lower() or 'fused' in k.lower() or 'fuse' in k.lower() or 'delta' in k.lower() or 'ssm' in k.lower():
            print('  **', k)
    # tensor 名
    names = []
